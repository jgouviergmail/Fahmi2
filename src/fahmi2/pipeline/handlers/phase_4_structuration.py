...
dans ``workspace/structured/{video_id}.md``.
"""
...
class Phase4StructurationHandler(PhaseHandler):
    ...
    def execute(
        ...
        *,
        ...
        """
        if video is None:
            raise ValueError("Phase4StructurationHandler requires a VideoExecution")
        started_at = utc_now()
        reformulated_text = _load_reformulated(ctx.workspace, video.video_id.value)
        ...
        out_path = ctx.workspace / _STRUCTURED_SUBDIR / f"{video.video_id.value}.md"
...
def _load_reformulated(workspace: Path, video_id: str) -> str:
    ...
    """
    path = workspace / _REFORMULATED_SUBDIR / f"{video_id}.md"
    if not path.exists():
        raise StorageError(
            code="STORAGE.REFORMULATED_MISSING",
            user_message=(
                f"Le contenu reformulé pour {video_id} est introuvable. "
